...
import com.txkj.contentbrowser2.BuildConfig;
	...
	private void includeAppFromPackage(MatrixCursor result, String docId, PackageInfo packageInfo,
		...
		if(showSystem == isSystemApp(appInfo)){
			...
			if (false) {
				...
				//FIXME:use appname
				row.add(Document.COLUMN_DISPLAY_NAME, appName);
			}
			row.add(Document.COLUMN_SUMMARY, summary);
	        row.add(Document.COLUMN_SIZE, size);
	        row.add(Document.COLUMN_MIME_TYPE, mimeType);
	...
	public static List<RunningAppProcessInfo> getRunningAppProcessInfo(Context ctx) {
		...
		if(Utils.hasOreo()){
			...
			for (PackageInfo packageInfo : allAppList) {
				...
				info.importance = RunningAppProcessInfo.IMPORTANCE_CACHED; //RunningAppProcessInfo.IMPORTANCE_BACKGROUND
			...
			for (RunningServiceInfo process : runningServices) {
				...
				info.importance = process.foreground ? RunningAppProcessInfo.IMPORTANCE_FOREGROUND : RunningAppProcessInfo.IMPORTANCE_CACHED; //RunningAppProcessInfo.IMPORTANCE_BACKGROUND
