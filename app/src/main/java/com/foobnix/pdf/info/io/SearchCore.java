...
import com.txkj.contentbrowser.LibraryFragment;
...
public class SearchCore {
    ...
    private static void search(File root, final List<String> exts, List<FileMeta> items) {
        ...
        for (File file : listFiles) {
		//FIXME:
            //if (file != null && file.getAbsolutePath() != null && file.getAbsolutePath().endsWith(".pdf")) {
            //    Log.d("find-files", file.getAbsolutePath());
            //}
            if (file.isHidden()) {
...
//FIXME:
                if (!LibraryFragment.FORCE_SKIP_NOMEDIA) {
